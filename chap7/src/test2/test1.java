...
class Apple extends Fruit {
	Apple(int price, double brix){
		super(price,brix);				// 상위 클래스 int, double과 순서 같아야함.
	}
...
class Buyer {
	...
	void buy(Food f){
		if(money < f.price) {
			System.out.println("비스킷 구매 시 잔액부족");
			return;			// 메서드 종료
		}
		...
		if(f instanceof Snack) {
			Snack s = (Snack)f;
			System.out.println(s.gram+"gram 상품 구매");		// 여긴 형변환 해줘야함
		}
		money -= f.price;
		point += f.point;
	...
	void summary() {
		...
		int frtot=0, dtot=0, stot=0;
		int fcnt = 0, dcnt = 0, scnt=0;
		String itemsName;
		String list = "",frlist="", dlist="", slist="";
		
		for(int i = 0; i<cnt; i++) {
			totAmt += cart[i].price;	
			totPoint += cart[i].point;
			
			if(cart[i] instanceof Fruit) {	// food 타입으로 fruit 접근 가능. cart[i]가 fruit 참조해라	
				frtot += cart[i].price;		// 과일 상품 가격 합
				frlist += cart[i]+",";		// 과일 상품 리스트
				fcnt++;
			}
		...
		System.out.println("구매한 물품 목록:"+totAmt);
		System.out.println("총 가격:"+totAmt);
		System.out.println("과일 구매금액:"+ frtot +",과일 구매건수:"+fcnt+", 과일 금액 리스트:"+frlist);
		System.out.println("음료 구매금액:"+ dtot + ", 음료 금액 리스트:"+dlist);
		System.out.println("과자 구매금액:"+ stot + ", 과자 금액 리스트:"+slist);
	}
}
